// Liquid metal WebGL2 five-pass dispersion pipeline from SylvaHero (inner-green-3d.html)
...
void main(){
  ...
  float pill = 1. - smoothstep(-0.5, 0.5, sd);
  ...
  o = vec4(min(rgb * pill, vec3(1.)), a);
}`;

export function mountLiquidMetal(host: HTMLElement): () => void {
  const cv = host.querySelector<HTMLCanvasElement>(".liquid-fx");
  const btn = host.querySelector<HTMLElement>(".liquid-button");
  if (!cv || !btn) return () => {};
  ...
  function drawTo(t: { fbo: WebGLFramebuffer; w: number; h: number } | null) {
  ...
  function frame(now: number) {
    ...
    const idle =
      ...
      press < 0.002 &&
      ptrAmt < 0.002;
    if (idle && now - lastDraw < 1000 / IDLE_HZ) {
      rafId = requestAnimationFrame(frame);
      return;
    }
    lastDraw = now;
  ...
  const sync = () => {
    ...
    host.classList.toggle("hot", hoverTarget > 0.5);
    host.classList.toggle("press", on.press);
  };
  ...
  btn.addEventListener("pointerenter", onPointerEnter as any);
  btn.addEventListener("pointerleave", onPointerLeave as any);
  ...
  return () => {
    ...
    btn.removeEventListener("pointerdown", onPointerDown as any);
