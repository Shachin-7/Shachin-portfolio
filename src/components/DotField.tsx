...
const DotField = memo(({
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      if (len > 0) {
        ...
        const crSq = cr * cr;
        const rad = Math.max(0.75, (p.dotRadius as number) / 2);
        ...
        for (let i = 0; i < len; i++) {
          ...
          if (distSq < crSq && eng > 0.01) {
            ...
            if (isBulge) {
              const tr = 1 - dist / cr;
              const push = tr * tr * (p.bulgeStrength as number) * eng;
  ...
  return (
    <div
      className={`dot-field-container relative w-full h-full overflow-hidden ${className}`}
